...
public class Test_HmacManager_VerifyAsync
{
    ...
    // public async Task Test_VerifyAsync_IsSuccess(HttpRequestMessage request)
    // {
    ...
    [TestCaseSource(typeof(TestCaseSource), nameof(TestCaseSource.GetHttpRequestMessages))]
    public async Task Test_VerifyAsync_IsFailure(HttpRequestMessage request)
    {
        var signingResult = await HmacManager.SignAsync(request);
        request.Headers.Authorization = new AuthenticationHeaderValue(request.Headers.Authorization.Scheme, $"{request.Headers.Authorization.Parameter}=");
        var verificationResult = await HmacManager.VerifyAsync(request);
        Assert.IsFalse(verificationResult.IsSuccess);
    }

    // [TestCaseSource(typeof(TestCaseSource), nameof(TestCaseSource.GetHttpRequestMessages))]
    // public async Task Test_VerifyAsync_Hmac_ShouldMatch_Verification(HttpRequestMessage request)
    // {
    //     var signingResult = await HmacManager.SignAsync(request);
    //     Assert.IsTrue(signingResult.IsSuccess);
    ...
    //     Assert.That(signingResult.Scheme, Is.EqualTo(verificationResult.Scheme));
